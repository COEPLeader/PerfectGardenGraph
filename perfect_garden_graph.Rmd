...
```{r, fig.height = 10, fig.width = 14, fig.alt = "wafdfmkls"}
  garden_harvest_maxbyday %>% 
...
```{r, fig.height = 10, fig.width = 14, fig.alt = "wafdfmkls"}
  maxbyday_plot <- garden_harvest_maxbyday %>% 
        ggplot(aes(y = weight_kg, x = date)) +
        geom_col(aes(fill = fct_reorder(vegetable, n), label = vegetable, group = weekday), position = "dodge") +
        scale_y_continuous(limits = c(0,50),expand = c(0, 0)) +
        scale_fill_hue() +
        #geom_text(aes(label = vegetable), angle = 90, check_overlap = TRUE, hjust = -.2) +
        labs(x= "",
  ...
  ggplotly(maxbyday_plot,
           tooltip = c("vegetable", "date", "weight_kg", "veg_total", "weekday"))
```
